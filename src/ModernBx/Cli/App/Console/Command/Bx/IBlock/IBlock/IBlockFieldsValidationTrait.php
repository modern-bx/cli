...
trait IBlockFieldsValidationTrait
{
    private function readFieldsArgumentOrStdin(mixed $fields): string
    {
        ...
        if ($fields !== null) {
            throw new \Exception(
                $this->trans('error.iblock.update_json_string'),
        ...
        if ($stdinIsTty) {
            throw new \Exception(
                $this->trans('error.iblock.fields_required'),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
    ...
    private function decodeFields(string $fields): array
    {
        ...
        if (json_last_error() !== JSON_ERROR_NONE) {
            throw new \Exception(
                $this->trans('error.iblock.update_invalid_json', ['%message%' => json_last_error_msg()]),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
        ...
        if (!$decoded instanceof \stdClass) {
            throw new \Exception(
                $this->trans('error.iblock.update_object'),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
    ...
    {
        $schemaPath = dirname(__DIR__, 3) . '/Validation/IBlockUpdateFields.schema.json';
        $schemaJson = file_get_contents($schemaPath);

        if ($schemaJson === false) {
            throw new \Exception(
                $this->trans('error.iblock.update_schema_read', ['%file%' => $schemaPath]),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
        ...
        if (json_last_error() !== JSON_ERROR_NONE || !is_array($schema) || array_is_list($schema)) {
            throw new \Exception(
                $this->trans('error.iblock.update_schema_invalid', ['%message%' => json_last_error_msg()]),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
        ...
        try {
            ...
            throw new \Exception(
                $this->trans('error.iblock.update_schema_invalid', ['%message%' => $exception->getMessage()]),
                static::CODE_INVALID_ARGUMENT_VALUE,
                $exception
            );
        }
        ...
        if ($errors !== []) {
            throw new \Exception(
                $this->trans('error.iblock.update_schema', ['%message%' => implode(PHP_EOL, $errors)]),
                static::CODE_INVALID_ARGUMENT_VALUE
            );
        }
    }
