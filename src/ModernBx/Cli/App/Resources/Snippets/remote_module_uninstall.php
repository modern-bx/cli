...
try {
    ...
    if (!\Bitrix\Main\ModuleManager::isModuleInstalled($moduleCode)) {
        echo CommandResult::successData(['warning' => 'MODULE_NOT_INSTALLED']);
        return;
    }
    ...
    if (!method_exists($installer, 'DoUninstall')) {
        throw new RuntimeException('Метод DoUninstall класса ' . get_class($installer) . ' не найден.');
    }
