...
final class RemoteIBlockPhpCodeBuilder
{
    ...
    {
        return $this->build('remote_iblock_get.php', [
            '$id = 0;' => '$id = ' . $id . ';',
        ]);
    }
    ...
    {
        return $this->build('remote_iblock_add.php', [
    ...
    {
        return $this->build('remote_iblock_delete.php', [
            '$id = 0;' => '$id = ' . $id . ';',
        ]);
    }
    ...
    {
        return $this->build('remote_iblock_update.php', [
            '$id = 0;' => '$id = ' . $id . ';',
            '$fields = [];' => '$fields = ' . var_export($fields, true) . ';',
        ]);
    }
    ...
    private function build(string $filename, array $replacements): string
    {
        ...
        if ($snippet === false) {
            throw new \RuntimeException(
                'Не удалось загрузить PHP-сниппет для удаленной операции с инфоблоком.'
            );
        }
