...
Можно передать один или несколько каталогов из допустимого списка. С `--remote` очистка выполняется на зарегистрированном удалённом проекте через PHP-консоль админки; корневые папки кеша сохраняются, удаляется только их содержимое.
